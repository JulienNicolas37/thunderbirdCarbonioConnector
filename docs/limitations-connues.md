...
- 🔴 **Pas d'ouverture de message : EN COURS, bloqué sur un problème non résolu.** Le pipeline complet est écrit et compile (`CarbonioProtocolHandler`, `CarbonioMessageChannel` hérité de `nsMailChannel`, `CarbonioService` implémentant `nsIMsgMessageService`) : résolution d'URI correcte (bug de masquage de membre C++ trouvé et corrigé), garde-fou anti-téléchargements concurrents en place et fonctionnel. **Symptôme restant** : `nsIMsgMessageService.loadMessage` est rappelée en boucle continue par le code JS de Thunderbird (`aboutMessage.js`), par paires, indéfiniment, même quand le téléchargement réussit à chaque fois sans erreur. Cause précise non identifiée — hypothèse la plus probable : un signal de complétion attendu par l'appelant JS (peut-être lié à `aUrlListener`, ignoré dans notre `LoadMessage`) ne lui parvient jamais, le poussant à réessayer. Piste `ExchangeUrl`/`nsIMsgMailNewsUrl` explorée et écartée (EWS lui-même ne l'instancie nulle part dans son propre code). **Prochaine piste à explorer** : poser la question directement à l'équipe Thunderbird (Matrix/Bugzilla, `MailNews Core :: Networking`) avec les logs de reproduction, plutôt que de continuer à deviner sans accès au code JS exact de cette version.
