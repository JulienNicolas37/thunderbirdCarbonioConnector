...
- 🔴 **Pas d'assistant graphique de création de compte** — création possible uniquement à la main via `prefs.js`/`user.js`. Pas de `CarbonioProtocolHandler` ni d'intégration UI au wizard.
  ...
  - **Patch complet rédigé, pas encore testé en conditions réelles** : `patches/add-carbonio-dedicated-account-wizard-screen.patch` — écran dédié à Carbonio dans l'assistant (nouvelle carte "Carbonio" sur l'écran de sélection, aux côtés d'IMAP/Microsoft/POP3 ; nouveau widget `email-carbonio-settings.mjs` + template associé ; câblage complet dans `views/email.mjs`), sur le modèle du chemin "Microsoft" mais simplifié en une seule étape (pas d'auto-discovery). 8 fichiers `comm-central` touchés au total (2 nouveaux, 6 modifiés), détail complet en en-tête du patch. **Procédure générale documentée** pour ce genre d'ajout : `docs/procedure-nouvelle-connexion-assistant-compte.md`. **Reste à faire** : appliquer le patch, builder, et valider la création d'un compte Carbonio de bout en bout via ce nouvel écran.
